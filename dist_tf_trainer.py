...
def setModelConfig():
    from autoencoder import autoencoderA, autoencoderB
    from autoencoder import encoder, decoderA, decoderB
    ...
    return autoencoderA, autoencoderB, encoder, decoderA, decoderB
...
def loadImages(directory, convert=None):
    imgPaths = [x.path for x in os.scandir(
        directory) if x.name.endswith(".jpg") or x.name.endswith(".png")]
    ...
    for i, image in enumerate(loadedImags):
        if i == 0:
            imags = numpy.empty((len(imgPaths), ) +
                                image.shape, dtype=image.dtype)
        imags[i] = image
    return imags


def updateWeights(encoder, decoderA, decoderB):
...
def getTrainingData(images, batchSize):
    ...
    for i, index in enumerate(indices):
        ...
        if i == 0:
            warpedImages = numpy.empty(
                (batchSize,) + warpedImg.shape, warpedImg.dtype)
            targetImages = numpy.empty(
                (batchSize,) + targetImg.shape, warpedImg.dtype)
...
def main():
    ...
    if FLAGS.job_name == 'ps':  # checks if parameter server
        ...
        # Graph
        if True:
            autoencoderA, autoencoderB, encoder, decoderA, decoderB = setModelConfig()
            imagesA = loadImages("data/A") / 255.0
            imagesB = loadImages("data/B") / 255.0

            imagesA += imagesB.mean(axis=(0, 1, 2)) - \
                imagesA.mean(axis=(0, 1, 2))
            ...
            for epoch in range(NUM_OF_EPOCHS):
                ...
                lossA = autoencoderA.train_on_batch(
                    warpedA, targetA)
                lossB = autoencoderB.train_on_batch(
                    warpedB, targetB)
                print(epoch, ': ', lossA, lossB)
                if epoch % 10 == 0:
                    logger.debug(str(lossA)+';'+str(lossB))
                    if epoch % 100 == 0:
                        updateWeights(encoder, decoderA, decoderB)
                        testA = targetA[0:14]
                        testB = targetB[0:14]
